...
public class RedisBuildBattleGamePropagatePartyEvent implements ServiceToClient {
    ...
    private JSONObject handleWarpEventWithGameValidation(PartyWarpResponseEvent warpEvent, List<UUID> participants) {
        ...
        BuildBattlePlayer warper = findPlayerByUuid(warperUUID);
        if (warper == null) {
        ...
        Game warperGame = TypeBuildBattleGameLoader.getPlayerGame(warper);
    ...
    private BuildBattlePlayer findPlayerByUuid(UUID uuid) {
        return HypixelGenericLoader.getLoadedPlayers().stream()
                .filter(p -> p.getUuid().equals(uuid))
                .filter(p -> p instanceof BuildBattlePlayer)
                .map(p -> (BuildBattlePlayer) p)
                .findFirst()
                .orElse(null);
    }
